...
class ClientFedCUP(Client):
    def __init__(self, global_dataset: Dataset, data_indices: List[int], local_model: nn.Module, client_id: int = 0, comm: Communicator = Communicator(), unlearning_indices: List[int] = [], args=None):
        """Federated Learning Client for Federated Unlearning Compensation Algorithm
        ...
        # v_i: momentum estimator of the remaining gradient (FedCUP), initialized lazily
    ...
    def _clip_global_norm(tensors: List[torch.Tensor], max_norm: float) -> List[torch.Tensor]:
        ...
        """
        total_norm = ClientFedCUP._global_norm(tensors)
    ...
    def local_unlearning(self, loader: Optional[DataLoader] = None, epoch: int = 0) -> None:
        """Local unlearning method for FedCUP (Federated Unlearning Compensation).
        ...
        if self.is_unlearner:
            ...
            if self.layerwise:
                ...
                diff_norm = ClientFedCUP._global_norm(diff)
                R = ClientFedCUP._global_norm(d) * (self.delta / lambda_i)
                if diff_norm > R:
                    ClientFedCUP._clip_global_norm(diff, R.item())
            ...
            # (with lambda_i = 0 the FedCUP update g_ri^{t+1} = v^{t+1} collapses to v)
            if self.momentum_estimator is None or self.grad_r_estimator is None:
                self.momentum_estimator = [g.clone() for g in grad_r]
