...
# FedURGE constricts the unlearning gradient per layer (layerwise) by default; 
...
for i in "${!DATASETS[@]}"; do
    ...
    echo "Running FedURGE on $DATASET (lr=$LR, delta=$DELTA, layerwise=$LAYERWISE, unlearn_epochs=$UNLEARN_EPOCHS, local_epochs=$LOCAL_EPOCHS)"
    for SEED in "${SEEDS[@]}"; do
        CUDA="${CUDA_AVAILABLE[$SEED % ${#CUDA_AVAILABLE[@]}]}"
        python -u unlearning.py --seed $SEED --config "config/${DATASET}.yml" --lr $LR \
            --unlearn_method FedURGE --delta $DELTA --layerwise 1 --batch_size $BATCH_SIZE \
            ...
            --cuda $CUDA > "logs/${DATASET}/FedURGE_${DATASET}_${SEED}.log" 2>&1 &
